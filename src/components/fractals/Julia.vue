...
const fracName = "Julia";
const xInit = ref(0.35);
const yInit = ref(0);
...
function fractalLogic(maxIterations, boundary, x, y) {
	...
	for (let i = 0; i < maxIterations; i++) {
		...
		let zx = Math.pow((Math.pow(cx, 2) + Math.pow(cy, 2)), (exponent.value / 2)) * Math.cos(exponent.value * Math.atan2(cy, cx)) + xInit.value;
		let zy = Math.pow((Math.pow(cx, 2) + Math.pow(cy, 2)), (exponent.value / 2)) * Math.sin(exponent.value * Math.atan2(cy, cx)) + yInit.value;
...
<template>
	<div v-description="'The initial X value to begin rendering the Julia fractal from'">
		<label for="xInit">Initial X point: </label>
		<input v-model="xInit" id="xInit" type="number" name="xInit" min="-1000" max="1000" />
	</div>
	<br />
	<div v-description="'The initial Y value to begin rendering the Julia fractal from'">
		<label for="yInit">Initial Y point: </label>
		<input v-model="yInit" id="yInit" type="number" name="yInit" min="-1000" max="1000" />
	</div>
	<br />
	<div v-description="'The value to use as the exponent in the fractal equasion: Z = Z^n'">
		<label for="exponent">Exponent: </label>
		<input v-model="exponent" id="exponent" type="number" name="exponent" min="-100" max="100" />
	</div>
	<br />
	<div v-description="'How far to zoom into the fractal. Larger values = deeper zoom'">
		<label for="zoom">Zoom: </label>
		<input v-model="zoom" id="zoom" type="number" name="zoom" />
	</div>
	<br />
	<div v-description="'How much to pan the rendering of the fractal on the X axis. Positive values = pan to the right'">
		<label for="xPan">X pan: </label>
		<input v-model="xPan" id="xPan" type="number" name="xPan" min="-100" max="100" />
	</div>
	<br />
	<div v-description="'How much to pan the rendering of the fractal on the Y axis. Positive values = pan down'">
		<label for="yPan">Y pan: </label>
		<input v-model="yPan" id="yPan" type="number" name="yPan" min="-100" max="100" />
	</div>
	<br />
	<div v-description="'The type of coloring function used when determining how to color the fractal'">
	...
	<br />
	<div v-description="'The color palette used to color the fractal based on the coloring function'">
	...
	<div id="footer">
		...
		<input id="renderButton" v-description="'Renders the fractal with all of its properties in the corrisponding panel'" @click="Render(canvas, context, iterationNum, boundaryNum)" type="button" value="Render" />
	</div>
</template>
